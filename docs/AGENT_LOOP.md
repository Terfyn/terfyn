...
| `constraints.maxIterations` | Counts **Generate** turns. Default **8**; unset/zero uses the default; values above the ceiling are clamped. The ceiling is the governing policy's `execution.maxIterations` (issue #522), or **32** when the policy sets none — so a policy can grant a coding agent more turns while cost/wall-clock still bound runaways. |
