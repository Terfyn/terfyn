...
var operationInputSchemas = map[string]json.RawMessage{
	...
	// git adapter
	"create_branch": json.RawMessage(`{"type":"object","properties":{"name":{"type":"string","description":"Branch name. Idempotent: if it already exists the op switches to it (not an error)."},"reset":{"type":"boolean","description":"Force-recreate the branch at base, discarding a prior attempt's commits and leftover uncommitted working-tree edits. Opt-in (destructive); requires base."},"base":{"type":"string","description":"Start point (ref/branch/sha) to create the branch from, e.g. \"main\". Required when reset is true; otherwise optional (defaults to the current HEAD)."}},"required":["name"],"additionalProperties":false}`),
	...
	"diff":          json.RawMessage(`{"type":"object","properties":{"paths":{"type":"array","items":{"type":"string"},"description":"Optional pathspecs to restrict the diff."},"base":{"type":"string","description":"Optional ref to diff against (e.g. \"main\"). Default is HEAD (working tree vs last commit)."},"staged":{"type":"boolean","description":"If true, diff the index (git diff --cached) instead of the working tree."}},"additionalProperties":false}`),
	"status":        json.RawMessage(`{"type":"object","properties":{"paths":{"type":"array","items":{"type":"string"},"description":"Optional pathspecs to restrict the status listing."}},"additionalProperties":false}`),
